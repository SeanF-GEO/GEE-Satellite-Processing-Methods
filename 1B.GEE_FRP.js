// Google Earth Engine Script to select GOES16 or GOES17 FRP rasters
...
for (var i = 0; i < n; i++) {
  ...
  var powerBand = image.select("Power");
  
  // Get the image ID
  var imageId = image.id().getInfo();
  
  // Export the "Power" band to Google Drive
  Export.image.toDrive({
    image: powerBand,
    description: imageId + "_Power", 
    fileNamePrefix: imageId + "_Power",
    crs: 'EPSG:32610', // Specify the CRS as UTM Zone 10N
    region: studyRegion, // Use the region specified in the "studyRegion" variable
    scale: 2000, // Set the scale to 2000 to maintain the native resolution
