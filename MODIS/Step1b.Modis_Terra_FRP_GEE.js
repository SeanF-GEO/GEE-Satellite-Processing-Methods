// Here is a Earth Engine script to export fire radiative power (FRP) and QA data for MODIS Terra
// Using The MODIS/061/MOD14A1 Dataset

// Define the region of interest (coordinates in order: [minLng, minLat, maxLng, maxLat])
// Change to the region you want to study
var region = ee.Geometry.Rectangle([-122.5, 41, -121, 42]);
...
function exportData(dataset, descriptionPrefix, date, region, frpBand, qaBand, frpScaleFactor, fireMaskBand) {
  ...
  for (var band in bandsToExport) {
    Export.image.toDrive({
      ...
      folder: 'GEE_FRP_Terra', // Folder name in Google Drive change to what you see fit
...
// Loop through each date to process and export Terra data
dateList.evaluate(function(dates) {
  dates.forEach(function(dateStr) {
    var date = ee.Date(dateStr); // Convert the date string back to an ee.Date object
    
    // Filter MOD14A1 (Terra) dataset for the current date
    var terraDataset = ee.ImageCollection('MODIS/061/MOD14A1')
    ...
    if (terraDataset) {
      // Process and export Terra data
      exportData(terraDataset, 'Terra', date, region, 'MaxFRP', 'QA', frpScaleFactor, 'FireMask');
    } else {
      print('No Terra data for date: ' + date.format('YYYY-MM-dd').getInfo()); // Print message if no data is available
    }
  });
});
